...
- 修改规则（MUST）：
  - `E_c(t)` 仅可通过 `depositEscrow` 单调增加，不得减少；
...
- SIA3：`depositEscrow(orderId, amount)` 要求 `amount > 0`，且 `escrow ← escrow + amount`（单调增加）。
...
- 函数：`createOrder, depositEscrow, acceptOrder, cancelOrder, markReady, approveReceipt, raiseDispute, requestForfeit, timeoutSettle, timeoutForfeit, acceptPriceBySig, withdrawPayout, withdrawRefund, extendDue, extendReview`。
